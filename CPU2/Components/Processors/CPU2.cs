...
namespace GSVM.Components.Processors
{
    ...
    {   
        Stack<GSVM.Constructs.DataTypes.IDataType> stack;
        public GSVM.Constructs.DataTypes.IDataType[] Stack
        {
            get
            {
        ...
        FastRegisters<Register> registers;

        Queue<Opcode> opcodeQueue = new Queue<Opcode>();
        Opcode opcode;
        Delegate operation;

        GSVM.Constructs.DataTypes.Integral<ushort> operandA, operandB;

        public string Name { get { return "GSVM CPU2"; } }
        ...
        public CPU2()
        {
            stack = new Stack<GSVM.Constructs.DataTypes.IDataType>();
            opcodes = new Dictionary<Opcodes, Delegate>();

            registers = new FastRegisters<Register>();
        ...
        public override void Tick()
        {
            ...
            if ((speedTesting) && (cycleCount == 30))
            {
                TimeSpan time = DateTime.Now - startTime;
                Speed = 30.0f / time.TotalSeconds;
                speedTesting = false;
            }
        }
